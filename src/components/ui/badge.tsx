  ...
  "inline-flex items-center rounded-full border font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        ...
        // Status variants
        pending: "bg-gradient-to-r from-amber-100 to-amber-200 text-amber-800 border-amber-200 shadow-sm",
        processing: "bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border-blue-200 shadow-sm",
        completed: "bg-gradient-to-r from-emerald-100 to-emerald-200 text-emerald-800 border-emerald-200 shadow-sm",
        failed: "bg-gradient-to-r from-red-100 to-red-200 text-red-800 border-red-200 shadow-sm",
        cancelled: "bg-gradient-to-r from-slate-100 to-slate-200 text-slate-800 border-slate-200 shadow-sm",
      },
      size: {
...
function Badge({
  ...
  const isStatusVariant = variant && ['pending', 'processing', 'completed', 'failed', 'cancelled'].includes(variant);
  ...
  const pulseAnimation = animated && variant === 'processing' ? {
    animate: {
      scale: [1, 1.05, 1],
      opacity: [1, 0.8, 1],
    },
    ...
    <>
      {showIcon && Icon && isStatusVariant && (
        <motion.div
          ...
          transition={variant === 'processing' ? { duration: 2, repeat: Infinity, ease: 'linear' } : {}}
          className="mr-1.5"
        >
