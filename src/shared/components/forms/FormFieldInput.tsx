...
const FormFieldInput = forwardRef<HTMLInputElement, FormFieldInputProps>(
  ({ label, error, leftIcon, rightIcon, fullWidth = true, className = '', ...props }, ref) => {
    const baseClasses =
      'block border border-gray-300 rounded-md px-3 py-2 bg-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent';
    const errorClasses = error ? 'border-red-300 focus:ring-red-500' : '';
    const iconPadding = leftIcon ? 'pl-10' : rightIcon ? 'pr-10' : '';

    const classes = [baseClasses, errorClasses, iconPadding, fullWidth ? 'w-full' : '', className]
      .filter(Boolean)
      .join(' ');
