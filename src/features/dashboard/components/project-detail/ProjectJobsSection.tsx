...
const JobCard: React.FC<JobCardProps> = React.memo(
  ({ job, onView, onStart, onCancel, onRetry }) => (
    <motion.div
    ...
    >
      <ParscadeCard className="p-6 h-full hover:shadow-lg transition-shadow">
        <div className="flex items-start justify-between mb-4">
          ...
          <Badge variant={getStatusColor(job.status)} showIcon animated={job.status === 'running'}>
            {job.status}
          </Badge>
        </div>
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Monitor and manage processing jobs for this project</p>
        </div>
      </div>
